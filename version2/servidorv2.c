...
void Cadena (TListaCon *lista, char cadenafinal [100])
{
	...
	sprintf(cadenafinal,"%d%s", lista->num, cadena);
}
...
void EdadColor(char p[80], char respuesta[200]) {
	...
	sprintf(respuesta,"%d",edad); //convertimos a string el entero "edad" y se lo copiamos a respuesta
...
void PorcentajeColor(char p[80], char respuesta[200])   {
	...
	sprintf(respuesta,"%.2f",tanto_porciento);
...
void *AtenderCliente(void *socket){
	...
	while (terminar ==0)
	{
		...
		else if (codigo == 7) //dame el socket
		{
			int res = DameSocket(&lista, nombre);
			if(res != -1){
				sprintf(respuesta,"%d",res);
			}else
			   sprintf(respuesta,"No hay socket para ese jugador");
		}
		else if (codigo == 8) //dame la posicion
		{
			DamePosicion(&lista, nombre);
		}
		else if (codigo == 5) //Cadena
		{
		...
		if ((codigo ==1)||(codigo==2)|| (codigo==3)|| (codigo==4)|| (codigo==6))
		{
